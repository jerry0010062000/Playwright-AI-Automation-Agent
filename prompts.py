...
OUTPUT_SPECIFICATIONS = {
    "natural": """
【回報語法：專業 Markdown 格式】
1. **合規矩陣表格**：條款 (SC), 名稱, 等級 (A/AA/AAA), 狀態 (PASS/FAIL/違規數), 具體描述。
2. **違規詳情**：提供具體 UI 位置（坐標或選擇器）、觀察現象、對身障者的影響。
3. **優化方向**：針對 FAIL 提供邏輯性開發建議。
4. **結束語**：跳過開場白與寒暄，內容寫完後請明確回覆結論以便系統結案。""",
    
    "json": "請以結構化的 JSON 物件回報結果，包含 status, violations 列表, 與 summary 三個核心欄位。"
}
