...
- 如果不確定，寧可多等待一會兒
- 發現錯誤時立即停止並回報
- 如果重複執行相同或類似的操作達 3 次（例如重複點擊相同坐標、重複按相同的鍵、重複滾動相同區域、或重複輸入相同網址）但頁面畫面或 URL 均無任何改變，表示操作已失效或遭遇限制。請立即終止執行，並向使用者回報你遇到的障礙，不要無止境地重複嘗試。
- 避免無休止地滾動頁面。如果連續滾動 3-4 次但內容完全沒有更新或改變，請立即停止滾動，改用其他方式（例如直接點擊、按 Tab 鍵尋找焦點、或直接呼叫 navigate 函數導航至目標 URL）。""",
...
4. 如果仍然失敗，回報問題""",
    ...
    # 無障礙動態審查
    "wcag_dynamic_guidelines": """
【無障礙動態審查與工具分工原則】
1. 靜態 Axe-core 檢測已經在本地 0-Token 模式下全量完成。在動態 AI 巡檢中【請勿重複呼叫 run_axe_audit】，請專注使用 scan_focus_path 進行 WCAG 2.1.1 (鍵盤操作)、2.1.2 (無陷阱)、2.1.4 (快捷鍵) 與 2.4.7 (焦點可見性 outline) 的視覺與焦點對照分析。
2. 當完成動態審查並產出最終總結報告時，【第 1 章必須先輸出包含 3 位數 Success Criteria (2.1.1, 2.1.2, 2.1.4, 2.2.1, 2.4.7, 2.5.3) 與 Level A/AA 等級的合規對照表格】，後續所有問題診斷與修復建議必須嚴格對照該表格進行！"""
}
