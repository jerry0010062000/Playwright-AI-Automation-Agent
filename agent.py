        ...
        try:
            with open(sitemap_path, "r", encoding="utf-8") as sf:
                ...
                if isinstance(raw_data, list) and len(raw_data) > 0:
                    ...
                    for node_path, node_info in raw_data["nodes"].items():
                        p = node_info.get("path") or node_path
                        full_u = urllib.parse.urljoin(base_url, p)
                        parsed_urls.append(full_u)
    ...
    for u in urls:
        ...
        try:
            ...
            try:
                ...
                print(f"[INFO] 客戶端選單點擊跳轉失敗: {nav_err}，改用真實頁面載入 (page.goto)。")
            ...
            # Session 保障衛士 (Session Guard)：自動檢查是否中途被路由器強制登出
            pw_input = page.query_selector("input[type='password']")
            if ("login" in page.url.lower() or (pw_input and pw_input.is_visible())) and "login" not in u.lower():
                print(f"  ⚠️ [SESSION GUARD] 檢測頁面 {u} 時發現 Session 掉線/自動登出！啟動自動重新登入...")
            ...
            # 過濾對應的 WCAG 條款 (若為 ALL 則包含所有靜態違規)
            matched_violations = []
            if wcag_ver.upper() == "ALL":
